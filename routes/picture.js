...
// 사진 업로드 API
router.post('',  authMiddleware.checkUser, uploadFile.any(), async (req, res, next) => {
    console.log(req);
...
// 유저 확인 필요
// 사진 리스트 확인 API
router.get('/',  authMiddleware.checkUser,async (req, res, next) => {
    // 사진 읽는거
...
// 사진 한장 확인 API
// 유저 확인 필요
router.get('/:pic_id',  authMiddleware.checkUser,async (req, res, next) => {
...
router.delete('/:pic_id', authMiddleware.checkUser, async (req, res, next) => {
    // 사진 삭제하는 로직
